...
Book.prototype.setBookForUser =  function (obj, callback) {
  ...
  var self = this;
  // once clear book data, after that insert data.
...
Book.prototype.updateBookForUser =  function (obj, callback) {
 ...
 var mongo = require('mongodb');
  ...
  mongo.Db.connect(mongoUri, function (err, db) {
    db.collection('test_insert', function(er, collection) {
      ...
      collection.update({"userId":obj.id, "bookInfo.bookStatus":true}, {$set: {"setting.targeEndDate":new Date(obj.date)}}, {w:1}, function(err, rs) {
        if(!err && rs === 1) {
          self.getBookForUser(obj.id, callback);
        }
      });
    });
  });
  ...
  mongo.Db.connect(mongoUri, function (err, db) {
    db.collection('test_insert', function(er, collection) {
      ...
      collection.update({"userId":userId}, {$set: {"bookInfo.bookStatus":false}}, {w:1, multi:true}, function(er,rs) {
        if(!err && rs > 0) {
          callback(null, {scssCode:"202", scssMsg: "Success delete User Books"});
        }
      });
    });
  });
