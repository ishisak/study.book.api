...
Book.prototype.setBookForUser =  function (obj, callback) {
  ...
  var self = this;
  self.findMongo('books', {bookKey: obj.num}, function(err, book) {
    if(book) {
      var data = {'userId':obj.id, bookInfo:book};
...
Book.prototype.resetBookForUser =  function (obj, callback) {
...
Book.prototype.ensureIndexes = function () {
  ...
  mongo.Db.connect(mongoUri, function (err, db) {
    db.collection('books', function(er, collection) {
      collection.ensureIndex( { "ts": 1}, {expireAfterSeconds: 60}, function(er,rs) {
        return;
      });
    });
  });
